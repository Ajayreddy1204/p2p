...
# # ------------------------------------------------------------
# # dashboard.py - Updated CSS with dynamic background, active category buttons, Prev/Next blue on click
# # ------------------------------------------------------------
...
#     /* Category buttons (Overdue, Disputed, Due) - active blue, inactive gray */
#     button[kind="primary"] {{
...
#     /* Invoice number buttons - default gray, hover/active blue with lift */
...
#     /* Prev/Next buttons - default gray, hover/active blue with lift */
...
#     # Get background color from session state (default white)
#     bg_color = st.session_state.get("dashboard_bg_color", "#ffffff")
#     inject_dashboard_css(bg_color)
#     # BG button: small circle at top right (outside the dashboard content)
#     # We'll place it in the main header area using columns
...
# # ------------------------------------------------------------
# # genie.py (unchanged, but included)
# # ------------------------------------------------------------
...
# # ------------------------------------------------------------
# # Response renderers (abbreviated, but full)
# # ------------------------------------------------------------
...
# # ------------------------------------------------------------
# # User question processing and Genie UI (abbreviated but full)
# # ------------------------------------------------------------
...
#     # Simple HTML table (static, no shaking)
#     summary_html = f"""
...
#     # Simple HTML table for status history
...
#             ]
#         # Simple HTML table for vendor info
...
#             ]
#         # Simple HTML table for company info
...
#     with col_search2:
#         st.markdown("<br>", unsafe_allow_html=True)  # small vertical alignment
...
#         if search_value:
#             # Try to find by invoice number first, then by PO number
...
#                 # Clear search state so we show the list
#                 st.session_state.inv_search_active = False
#                 st.session_state.inv_search_term = ""
    
#     # If we are here, show the invoice list
...
#     # BG button: circle button on the right-hand side (top right)
#     # We'll add it as a small button in the header row
#     # Create a custom header with columns: title, nav, BG button
#     st.markdown("""
# <style>
# .block-container {
...
#     # Create three columns: title (1.2), nav (2.5), right area (1) containing logo and BG button
...
#     with col_right:
#         # Logo and BG button side by side
...
#         with col_bg:
#             # BG circle button: when clicked, show a color picker in a popover (or use a simple dialog)
#             # Using a button that opens a popover with color picker (Streamlit popover not available, so we use a small form with color picker)
#             # Simpler: a button that toggles a color picker in a sidebar? Better: add a color picker in the right column but make it small.
#             # To meet "click on that it need to show line in that it need to show colours" - we'll use st.popover (Streamlit 1.29+)
#             # Use a popover if available, else fallback to a small color picker next to button.
#             # Since we can't guarantee version, we'll use a st.popover (newer) but also fallback.
#             try:
#                 with st.popover("🎨 BG"):
#                     new_color = st.color_picker("Pick background color", st.session_state.dashboard_bg_color)
#                     if new_color != st.session_state.dashboard_bg_color:
#                         st.session_state.dashboard_bg_color = new_color
#                         st.rerun()
#                 # Show a circle button that opens popover
#                 st.markdown(f'<div class="bg-button" onclick="document.getElementById(\'bg_popover_trigger\').click();">🎨</div>', unsafe_allow_html=True)
#                 # Hidden button to trigger popover (workaround)
#                 # Actually, st.popover creates a button automatically. So we just use st.popover as is.
#                 # But we want a circle button. We'll override style.
#                 # Better: use st.popover with a custom label that is styled as circle.
#                 with st.popover("🎨"):
...
#                         st.rerun()
#             except:
#                 # Fallback: simple color picker in the column
...
#     # Now render the appropriate page with the dynamic background
#     # Note: inject_dashboard_css is called inside each render function, but we need to pass the current bg color.
#     # For Dashboard, we pass the color inside render_dashboard. For other pages, we may also want to apply background but it's optional.
#     # We'll modify render_dashboard to take the bg color from session state (already done).
#     # For other pages, we can also apply the same background if desired.
#     if st.session_state.page == "Dashboard":
#         render_dashboard()
#     elif st.session_state.page == "Genie":
#         # For Genie and others, we can also apply the same background by injecting CSS
#         # But to keep it simple, we just call the render functions; they have their own CSS.
#         # However, we want the background to apply globally. We'll add a global CSS inject at the start of each render or in main.
#         # Since inject_dashboard_css is specific to dashboard, we add a simple global background for other pages.
#         # We'll add a global background style here.
#         st.markdown(f"""
#         <style>
#             .main > .block-container {{
...
# ------------------------------------------------------------
# dashboard.py - Updated CSS for button hover effects and equal height charts
# ------------------------------------------------------------
def inject_dashboard_css(bg_color: str = "#ffffff"):
    st.markdown(f"""
<style>
