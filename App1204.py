...
<style>
    ...
    /* Invoice number buttons - blue background */
    button[data-testid^="baseButton-na_card_"] {{
        background-color: #2563eb !important;
        background: #2563eb !important;
        border: 2px solid #2563eb !important;
        ...
        if isinstance(date_range, (list, tuple)) and len(date_range) == 2:
            ...
            if (new_start, new_end) != (rng_start, rng_end):
                if not st.session_state.get("_preset_clicked", False):
                    ...
                    st.session_state._preset_clicked = False
    with col_vendor:
        # Always fetch fresh vendor list for current date range
        vendor_sql = f"""
            SELECT DISTINCT v.vendor_name
            FROM {DATABASE}.fact_all_sources_vw f
            LEFT JOIN {DATABASE}.dim_vendor_vw v ON f.vendor_id = v.vendor_id
        ...
        """
        vendors_df = run_query(vendor_sql)
        vendor_names = vendors_df["vendor_name"].tolist() if not vendors_df.empty else []
        # Remove duplicates and add "All Vendors" only once at the beginning
        unique_vendors = list(dict.fromkeys(vendor_names))  # Preserve order, remove duplicates
        vendor_list = ["All Vendors"] + unique_vendors
        # Find index for current selection
        if selected_vendor in vendor_list:
            vendor_index = vendor_list.index(selected_vendor)
        else:
            vendor_index = 0
        selected = st.selectbox(
            "Select vendor",
                    ...
                    with col:
                        with st.container(border=True):
                            ...
                            with left:
                                ...
                                ref = format_invoice_number(ref)
                                btn_key = f"na_card_{start_idx}_{card_global_idx}_{ref.replace(' ', '_')[:30]}"
                                # Invoice button – blue background via CSS
...
/* Make header row items vertically aligned */
