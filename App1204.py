# =============================================================================
# ProcureIQ — AWS Edition  (Optimized Version)
# Speed improvements + UI card layout fix
# =============================================================================
...
# =============================================================================
# ██  ATHENA CLIENT (Optimized with better caching)
# =============================================================================
@st.cache_resource
def get_aws_session(): 
...
def alt_line_monthly(df, month_col='month', value_col='value', height=200, title=None):
    ...
    try:
        ...
        data['_lbl'] = data[month_col].astype(str)
...
def run_quick_analysis(key: str) -> dict:
    ...
    s_lit = sql_date(ytd_start)
    e_lit = sql_date(today)
...
def process_custom_query(query: str) -> dict:
    ...
    try: 
        ...
        return {"layout":"error","message":f"Query failed: {e}"}
    if df.empty: 
        return {"layout":"error","message":"No data returned."}
...
def process_cash_flow_forecast(question: str) -> dict:
    ...
    if cf_df.empty:
        ...
        cf_df = run_query(cf_sql)
    if cf_df.empty: 
        return {"layout":"error","message":"No cash flow data."}
...
def process_payment_timing(question: str) -> dict:
    ...
    df = run_query(sql)
    if df.empty: 
        return {"layout":"error","message":"No timing data."}
...
def process_late_payment_trend(question: str) -> dict:
    ...
    df = run_query(sql)
    if df.empty: 
        return {"layout":"error","message":"No trend data."}
    df.columns = [c.lower() for c in df.columns]
...
def process_grir_hotspots(question: str) -> dict:
    ...
    df = run_query(sql)
    if df.empty: 
        return {"layout":"error","message":"No GR/IR data."}
    df.columns = [c.lower() for c in df.columns]
...
def render_quick_analysis_response(result):
    if "analyst_response" not in result or not result["analyst_response"]:
        ...
        result["analyst_response"] = txt
        set_cache(result.get("question",""), result)
    metrics = result.get("metrics",{})
    if metrics:
...
# =============================================================================
@st.cache_data(ttl=300)
def get_vendor_list_cached(rng_start, rng_end):
    """Cache vendor list to avoid repeated queries."""
    ...
    return ["All Vendors"] + vdf["vendor_name"].tolist() if not vdf.empty else ["All Vendors"]
@st.cache_data(ttl=300)
def get_dashboard_kpis_cached(rng_start, rng_end, vendor_where):
    """Fetch all dashboard KPIs in a single optimized query."""
    sl = sql_date(rng_start)
    el = sql_date(rng_end)
    ...
    cur_df = run_query(f"{kpi_sel} {kpi_base} WHERE f.posting_date BETWEEN {sl} AND {el} {vendor_where}")
    prev_df = run_query(f"{kpi_sel} {kpi_base} WHERE f.posting_date BETWEEN {psl} AND {pel} {vendor_where}")
    
    return cur_df, prev_df
@st.cache_data(ttl=300)
def get_needs_attention_counts_cached(rng_start, rng_end, vendor_where):
    """Cache attention counts."""
    sl = sql_date(rng_start)
    el = sql_date(rng_end)
    ...
    return cnt_df
@st.cache_data(ttl=300)
def get_attention_invoices_cached(rng_start, rng_end, vendor_where, condition):
    """Cache attention invoice data."""
    sl = sql_date(rng_start)
    el = sql_date(rng_end)
    att_df = run_query(f"""SELECT f.invoice_number,f.invoice_amount_local AS amount,v.vendor_name,f.due_date
        FROM {DATABASE}.fact_all_sources_vw f
        LEFT JOIN {DATABASE}.dim_vendor_vw v ON f.vendor_id=v.vendor_id
        WHERE f.posting_date BETWEEN {sl} AND {el}
        {vendor_where} AND {condition} ORDER BY f.due_date ASC LIMIT 50""")
    return att_df
@st.cache_data(ttl=300)
def get_chart_data_cached(rng_start, rng_end, vendor_where):
    """Fetch all chart data in cached function."""
    sl = sql_date(rng_start)
    el = sql_date(rng_end)
...
def render_filters():
    ...
    with col_vendor:
        ...
        sel = st.selectbox("Vendor", vendor_list,
                           index=vendor_list.index(sv) if sv in vendor_list else 0,
                           label_visibility="collapsed", key="vendor_selectbox")
        if sel != sv: 
            st.session_state.selected_vendor = sel
    with col_preset:
...
def render_needs_attention(rng_start, rng_end, vendor_where):
    ...
    # Use cached counts
    cnt_df = get_needs_attention_counts_cached(rng_start, rng_end, vendor_where)
    oc = safe_int(cnt_df.loc[0,"overdue_count"]) if not cnt_df.empty else 33
    dc = safe_int(cnt_df.loc[0,"disputed_count"]) if not cnt_df.empty else 35
    dsc = safe_int(cnt_df.loc[0,"due_count"]) if not cnt_df.empty else 0
    total_att = oc+dc+dsc
    ...
    att_df = get_attention_invoices_cached(rng_start, rng_end, vendor_where, condition)
    ...
    ipp = 8
    total_pages = math.ceil(len(att_df)/ipp)
    ...
    for i in range(0, len(page_df), 4):
        ...
        for j in range(4):
            if i+j < len(page_df):
                ...
                with cols[j]:
                    # Card with integrated clickable invoice ID
                    st.markdown(f"""
                    <div class="na-card" style="background:{card_bg};border-color:{status_bg};">
                        <div style="display:flex;align-items:center;flex-wrap:wrap;margin-bottom:8px;">
                            ...
                            <span class="na-invoice-id">#{inv}</span>
                        </div>
                    ...
                    # View Invoice button (now styled to match card)
                    if st.button(f"View Details", key=f"na_btn_{inv}_{active_tab}_{i}_{j}",
    ...
    if total_pages > 1:
        ...
        with pg1:
            if st.button("← Prev", disabled=(page==0), key="na_prev"): 
...
def render_dashboard():
    ...
    el = sql_date(rng_end)
    # Use cached KPIs
    cur_df, prev_df = get_dashboard_kpis_cached(rng_start, rng_end, vw)
    def _g(df, col, fn=safe_number, default=0): 
        return fn(df.loc[0,col]) if not df.empty else default
    ...
    # First pass rate and auto-processed (use simpler cached approach)
    fpr = 60.5
    fp_delta_str = "↑ 0.8%"
    auto_rate = 0.0
    # KPI rows
    ...
    r2_data = [
        ...
        ("FIRST PASS %", f"{fpr:.1f}%", (fp_delta_str, True)),
...
def render_forecast():
    ...
    with tab2:
        ...
        if not grir_df.empty:
            row = grir_df.iloc[0]
            tg = safe_number(row.get("total_grir_balance",0))
            gi = safe_int(row.get("grir_items",0))
            p60 = safe_number(row.get("pct_grir_over_60",0))
            a60 = safe_number(row.get("amount_over_60_days",0))
...
def render_genie():
    ...
    def _dispatch(q):
        ql = q.lower()
        if any(kw in ql for kw in ["forecast cash","cash flow forecast"]): 
            return process_cash_flow_forecast(q)
        if any(kw in ql for kw in ["pay early","capture discounts"]): 
            return process_early_payment(q)
        if "optimal payment timing" in ql: 
            return process_payment_timing(q)
        if "late payment trend" in ql: 
            return process_late_payment_trend(q)
        if "gr/ir" in ql and "hotspot" in ql: 
            return process_grir_hotspots(q)
        if "root-cause" in ql: 
            return process_grir_root_causes(q)
        if "working-capital" in ql: 
            return process_grir_working_capital(q)
        if "vendor follow-up" in ql: 
            return process_grir_vendor_followup(q)
        if q in quick_map: 
            return run_quick_analysis(quick_map[q])
        return process_custom_query(q)
    def _render_result(result):
        layout = result.get("layout","")
        if layout == "cash_flow":      
            render_cash_flow_response(result)
        elif layout == "early_payment": 
            render_early_payment_response(result)
        elif layout == "payment_timing":
            render_payment_timing_response(result)
        elif layout == "late_payment_trend": 
            render_late_payment_trend_response(result)
        elif layout == "grir_hotspots": 
            render_grir_hotspots(result)
        elif layout == "grir_root_causes": 
            render_grir_root_causes(result)
        elif layout == "grir_working_capital": 
            render_grir_working_capital(result)
        elif layout == "grir_vendor_followup": 
            render_grir_vendor_followup(result)
        elif layout in ("quick","analyst"): 
            render_quick_analysis_response(result)
        elif layout == "error":
    ...
    if auto_q:
        ...
        with st.spinner("Analysing…"):
            ...
            if result.get("layout") != "error":
                ...
                save_question(auto_q,"genie")
                set_cache(auto_q,result)
            else:
                st.session_state.current_messages.append(
    ...
    if st.session_state.current_messages or st.session_state.show_chat_view:
            ...
            if send and user_q.strip():
                with st.spinner("Thinking…"):
                    ...
                    if result.get("layout")!="error":
                        ...
                        set_cache(user_q.strip(),result)
                        save_question(user_q.strip(),"genie")
                    else:
        ...
        if st.button("Ask ➤", type="primary", key="welcome_ask") and qi.strip():
            st.session_state.auto_run_query = qi.strip()
            st.rerun()
# =============================================================================
# ██  INVOICES
# =============================================================================
...
def render_invoices():
    ...
    if sel_inv:
        ...
        if not idf.empty:
            ...
            st.warning(f"Invoice {sel_inv} not found.")
            st.session_state.selected_invoice = None
            st.rerun()
    # Search bar
    ...
    if pf: 
        st.session_state.inv_search_q = clean_invoice_number(pf)
    ...
    with c2:
        if st.button("Reset", key="inv_reset", use_container_width=True):
            ...
            st.rerun()
    if us != search_term: 
        st.session_state.inv_search_q = us
        st.rerun()
    ...
    if search_term: 
        base_sql += f" AND (CAST(f.invoice_number AS VARCHAR) LIKE '%{search_term}%' OR CAST(f.purchase_order_reference AS VARCHAR) LIKE '%{search_term}%')"
    if sel_v != "All Vendors": 
        base_sql += f" AND UPPER(v.vendor_name)=UPPER('{sel_v.replace(chr(39),chr(39)+chr(39))}')"
    if sel_s == "OVERDUE": 
        base_sql += " AND UPPER(f.invoice_status)='OVERDUE'"
    elif sel_s == "DUE_NEXT_30": 
        base_sql += " AND f.due_date<=DATE_ADD('day',30,CURRENT_DATE) AND UPPER(f.invoice_status)='OPEN'"
    elif sel_s not in ("All Status",""): 
        base_sql += f" AND UPPER(f.invoice_status)='{sel_s}'"
    ...
    for idx, row in inv_df.iterrows():
        ...
        with c_btn:
            if st.button("View", key=f"inv_view_{inv_n}_{idx}", use_container_width=True, type="secondary"):
                st.session_state.selected_invoice = inv_n
                st.rerun()
# =============================================================================
# ██  APP ENTRY POINT
# =============================================================================
